  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-32">
            {residentServices.map((service, idx) => (
              <div 
                key={idx} 
                className="bg-white p-12 border border-[#E1E8F0] shadow-trust rounded-[28px] hover:shadow-premium transition-all duration-520 group animate-in fade-in slide-in-from-bottom-8"
                style={{ animationDelay: `${0.1 * idx}s`, animationFillMode: 'both' }}
              >
                <div className="w-16 h-16 bg-brand-gray rounded-[18px] flex items-center justify-center text-brand-navy mb-10 group-hover:bg-brand-amber transition-all duration-520">
                  <div className="w-8 h-8 group-hover:scale-110 transition-transform">
                    {service.icon}
                  </div>
                </div>
                <h3 className="text-2xl font-bold text-brand-navy mb-5 tracking-tight group-hover:text-brand-amber transition-colors">{service.title}</h3>
                <p className="text-gray-500 mb-10 leading-relaxed font-medium opacity-80 text-[15px]">
                  {service.description}
                </p>
                <div className="pt-8 border-t border-[#F0F4F8]">
                  <button className="text-brand-navy font-bold text-[11px] uppercase tracking-[0.2em] flex items-center gap-3 hover:text-brand-amber transition-all group/btn">
                    Iniciar Gestión
          ...
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700" style={{ animationDelay: '0.4s' }}>
            <div className="text-center mb-20">
              <span className="text-brand-amber font-bold uppercase tracking-[0.3em] text-[11px] mb-6 block">Soporte y Consultas</span>
            ...
            <div className="space-y-6">
              {faqs.residentes.map((faq, idx) => (
                <div key={idx} className="bg-white p-8 md:p-12 rounded-[28px] border border-[#E1E8F0] shadow-trust hover:shadow-premium transition-all duration-520 group">
      ...
      <section className="py-24 bg-brand-gray/30">
        <div className="container mx-auto px-4">
          <div className="bg-brand-navy p-12 md:p-24 rounded-[32px] md:rounded-[40px] text-white relative overflow-hidden shadow-trust text-center animate-in fade-in zoom-in-95 duration-700">
            {/* Background Accents */}
            ...
            <div className="relative z-10 max-w-2xl mx-auto">
              <span className="inline-block text-brand-amber font-bold uppercase tracking-[0.3em] text-[11px] mb-8">Asistencia Directa</span>
